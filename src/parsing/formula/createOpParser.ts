...
const createParser = <B, R extends B>(rules: IOp<B, B>[]): IOpParser<B, R> => ({
    a: rule => createParser<any, any>([...rules, rule]),
    finish: () => {
        ...
        const parser: Parser<B> = precedenceGrouped.reduce<Parser<B>>(
            (nextParser, rules, i) => {
                ...
                const baseRules = rules
                    .filter(
                        (op): op is IBaseOp<any> | IPrefixOp<any, any> =>
                            op.t == "b" || op.t == "p"
                    )
                    .map(({op, t}) =>
                        t == "b"
                ...
                const rightInfixRules = rules
                    .filter(
                        (op): op is IInfixOp<any, any> => op.t == "i" && op.ass == "right"
                    )
                    .map(({op}) => op);
                ...
                const leftRecursiveRules = rules
                    .filter(
                        (op): op is IInfixOp<any, any> | ISuffixOp<any, any> =>
                            (op.t == "i" && op.ass != "right") || op.t == "s"
                    )
                    .map(({op, t}) =>
                        t == "i"
                            ? P.seq(op, rightRecursiveParser)
                            : op.map(out => [out] as const)
                    );
                const leftRecursiveParser =
                    leftRecursiveRules.length > 0
                        ? P.seq(
                              ...
                              P.alt<
                                  | [(left: B, right: B) => R, B]
                                  | readonly [(left: B, right: B) => R]
                              >(...leftRecursiveRules).many()
                          ).map(([first, rest]) =>
                              rest.reduce(
                                  (left, [combiner, right]) => combiner(left, right!),
                                  first
                              )
                          )
                        : rightRecursiveParser;
