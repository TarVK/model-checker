...
import {IEditArc} from "./_types/IEditArc";
...
export class LTSGraphState {
    ...
    public LTSState: State;

    /**
     * Creates a new graph state for a given LTS state
     * @param LTSState The state that the graph state is for
     */
    public constructor(LTSState: State) {
    ...
    /**
     * Sets the arc being edited (its action)
     * @param arc The arc to be edited
     */
    public setEditingArc(arc: null | IEditArc): void {
        this.editingArc.set(arc);
    }

    /**
     * Retrieves the currently selected arc
     * @param hook The hook to subscibe to changes
     * @returns The currently selected arc
     */
    public getEditingArc(hook?: IDataHook): null | IEditArc {
